...
if "messages" not in st.session_state:
    if os.path.exists(chat_file):
        with open(chat_file, "r") as f: st.session_state.messages = json.load(f)
    else:
    ...
    with st.chat_message("assistant"):
        ...
        for _ in range(len(keys)):
            try:
                ...
                # Use limited history to avoid token walls
        ...
        if not success:
            st.warning("Connection limit reached. Re-syncing power sources in 60s...")import streamlit as st
import google.generativeai as genai
import json
import os
